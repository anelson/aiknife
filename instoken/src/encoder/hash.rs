...
impl TokenEncoder {
    ...
    pub fn encode(&self, bytes: &[u8]) -> Option<TokenInt> {
        self.0.get(bytes).copied()
    }
...
impl TokenDecoder {
    ...
    pub fn decode(&self, token: TokenInt) -> Option<&TokenString> {
        self.0.get(&token)
    }
    ...
    pub fn token_len(&self, token: TokenInt) -> Option<usize> {
        self.decode(token).map(TokenString::len)
    }
}
